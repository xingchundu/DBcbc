...
public abstract class Analyser {
	...
	protected List<Table> listTables(Connection connection) throws SQLException {
		...
		// 二级索引(非主键)抽取: 由子类 getSecondaryIndexDefines 按各库数据字典实现
		logger.info("开始获取二级索引信息");
		List<IndexDefinition> secondaryIdxList = this.getSecondaryIndexDefines(connection, catalog, schema, tableMap);
		this.mergeSecondaryIndexes(tableMap, secondaryIdxList);
		logger.info("二级索引信息归并完成,共{}个", secondaryIdxList == null ? 0 : secondaryIdxList.size());
	...
	/**
	 * 统计当前库中的表与二级索引数量（不输出分析过程日志，供迁移完成汇总使用）
	 */
	public ObjectCounts countTablesAndSecondaryIndexes(Connection connection) throws SQLException {
		String catalog = connection.getCatalog();
		String schema = connection.getSchema();
		List<Table> tableDefinesList = this.getTableDefines(connection, catalog, schema);
		int tableCount = tableDefinesList == null ? 0 : tableDefinesList.size();
		Map<String, Table> tableMap = new HashMap<>();
		if (tableDefinesList != null) {
			for (Table table : tableDefinesList) {
				tableMap.put(table.getTableName(), table);
		...
		List<IndexDefinition> secondaryIdxList = this.getSecondaryIndexDefines(connection, catalog, schema, tableMap);
		int indexCount = secondaryIdxList == null ? 0 : secondaryIdxList.size();
	...
	/**
	 * 将按表归类的二级索引挂到表对象上(仅非主键索引)
	 */
	private void mergeSecondaryIndexes(Map<String, Table> tableMap, List<IndexDefinition> indexList) {
		if (indexList == null) {
			return;
		}
		for (IndexDefinition idx : indexList) {
			Table t = tableMap.get(idx.getTableName());
			if (t != null) {
				t.addSecondaryIndex(idx);
			}
		}
	}

	/**
	 * 从源库拉取非主键二级索引. 默认无; Oracle/MySQL/PostgreSQL 子类覆写. tableMap 用于排除与主键列序完全一致的 Oracle 主键支持索引
	 */
