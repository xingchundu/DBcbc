...
public abstract class BaseTypeConverter {
	...
	/**
	 * Oracle 源端未在映射表中定义时的目标类型（PostgreSQL 等应覆盖为 TEXT，避免生成非法的 CLOB）
	 */
	...
	/**
	 * 映射表无对应项时，按源列所属库类型给定的默认目标类型；子类可覆盖（如 MySQL→Oracle 用 CLOB）
	 */
	protected String getUnmappedFallbackType(Column column) {
		if (column.getDataBaseType().equals(DataBaseType.ORACLE)) {
			return getOracleUnmappedFallbackType();
		}
		return "TEXT";
	}

	/**
	 * 将 JDBC/字典中的类型名转为映射表键（如 TIMESTAMP(6)、NUMBER(10,0) → 基名）
	 */
	private String resolveTypeMappingKey(String dataType) {
		if (StringUtil.isBlank(dataType)) {
			return null;
		}
		String u = dataType.toUpperCase(Locale.ROOT).trim();
		if (typeMapping.containsKey(u)) {
			return u;
		}
		int paren = u.indexOf('(');
		if (paren > 0) {
			String base = u.substring(0, paren).trim();
			if (typeMapping.containsKey(base)) {
				return base;
			}
		}
		return u;
	...
	public DataBaseDefine convert(DataBaseDefine dataBaseDefine) {
		...
		for (Table table : tables) {
			...
			for (Column column : columns) {
				String mapKey = resolveTypeMappingKey(column.getDataType());
				String columnType = mapKey == null ? null : typeMapping.get(mapKey);
				StringBuilder stringBuilder = new StringBuilder("");
				if (mapKey != null && typeMapping.containsKey(mapKey)) {
					// 配置了映射关系
					...
					// 未配置映射关系
					column.setFinalConvertDataType(getUnmappedFallbackType(column));
