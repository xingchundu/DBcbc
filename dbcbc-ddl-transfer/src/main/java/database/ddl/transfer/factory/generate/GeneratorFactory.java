...
import database.ddl.transfer.factory.generate.impl.SqlServerGenerator;
	...
	public static Generator getInstance(Connection connection, DataBaseDefine dataBaseDefine, DBSettings targetSettings) {
		try {
			...
			if (typeLower.contains("mysql")) {
				...
				generator = new SqlServerGenerator(connection, dataBaseDefine, targetSettings);
			} else if (typeLower.contains("dm") || typeLower.contains("dameng")) {
				// DM 高度兼容 Oracle，复用 OracleSqlGenerator
				generator = new OracleSqlGenerator(connection, dataBaseDefine, targetSettings);
			} else {
