...
public class PostgreSqlAnalyser extends Analyser {
	...
	protected List<Column> getColumnDefines(Connection connection, String catalog, String schema) {
		String sql = "select c.relname,a.attname,case a.atttypid when 21 /*int2*/ then 16 when 23 /*int4*/ then 32 when 20 /*int8*/ then 64 when 1700 /*numeric*/ then case when atttypmod = -1 then null else ((atttypmod - 4) >> 16) & 65535  end " + 
				...
				"a.attnotnull AS notnull,pg_get_expr(n.adbin, n.adrelid) AS adsrc,b.description AS comment,q.contype from pg_class c,pg_attribute a " + 
...
//		column.setExtra(resultSet.getString("adsrc"));
	...
	/**
	 * 从 JDBC getIndexInfo 拉取 public 下非主键支撑索引(含 btree/gin 等,列序由 ORDINAL_POSITION 保证)
	 */
	@Override
	protected List<IndexDefinition> getSecondaryIndexDefines(Connection connection, String catalog, String schema,
		...
		// 与 getTableDefines 中 schemaname 一致(默认 public)
		String sch = (schema != null && !schema.isEmpty()) ? schema : "public";
		for (Map.Entry<String, Table> e : tableMap.entrySet()) {
			try {
				all.addAll(JdbcSecondaryIndexReader.readForTable(connection, catalog, sch, e.getValue(), e.getKey()));
			} catch (SQLException ex) {
				throw new RuntimeException("获取PostgreSQL二级索引失败: " + e.getKey(), ex);
			}
		}
		// 用 pg_am 补全 GIN/BTREE 等(覆盖 JDBC 默认的 btree 占位)
		enrichPostgresWithPgAm(connection, sch, all);
		return all;
	}

	/**
	 * 从系统表补充索引访问方法(非主键、普通表)
	 */
	private void enrichPostgresWithPgAm(Connection connection, String schema, List<IndexDefinition> all) {
		if (all == null || all.isEmpty()) {
			return;
		}
		PreparedStatement ps = null;
		ResultSet rs = null;
		Map<String, String> keyToAm = new HashMap<>();
		try {
			String sql = "SELECT lower(c.relname) AS t, lower(i.relname) AS idx, lower(a.amname) AS am "
					+ "FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace AND n.nspname = ? "
					+ "JOIN pg_index x ON x.indrelid = c.oid AND NOT x.indisprimary "
					+ "JOIN pg_class i ON i.oid = x.indexrelid AND i.relkind = 'i' "
					+ "JOIN pg_am a ON a.oid = i.relam " + "WHERE c.relkind = 'r'";
			ps = connection.prepareStatement(sql);
			ps.setString(1, schema);
			rs = ps.executeQuery();
			while (rs.next()) {
				String k = rs.getString("t") + "\0" + rs.getString("idx");
				keyToAm.put(k, rs.getString("am"));
			}
		} catch (Throwable e) {
			throw new RuntimeException("补充 PostgreSQL 索引访问方法失败", e);
		} finally {
		...
		for (IndexDefinition d : all) {
			...
			if (keyToAm.containsKey(k)) {
				d.setAccessMethod(keyToAm.get(k));
			}
		}
	}
}
