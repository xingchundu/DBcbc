...
import database.ddl.transfer.factory.analyse.impl.DmAnalyser;
import database.ddl.transfer.factory.analyse.impl.SqlServerAnalyser;
...
public final class AnalyserFactory {
	...
	public static Analyser getInstance(Connection connection) {
		try {
			...
			String typeLower = dataBaseType == null ? "" : dataBaseType.toLowerCase(Locale.ROOT);
			if (typeLower.contains("mysql")) {
				analyser = new MySqlAnalyser(connection);
			} else if (typeLower.contains("postgres")) {
				analyser = new PostgreSqlAnalyser(connection);
			} else if (typeLower.contains("oracle")) {
				analyser = new OracleSqlAnalyser(connection);
			} else if (typeLower.contains("dm") || typeLower.contains("dameng")) {
				analyser = new DmAnalyser(connection);
			} else if (typeLower.contains("microsoft sql server") || typeLower.contains("sql server")) {
				analyser = new SqlServerAnalyser(connection);
			} else {
