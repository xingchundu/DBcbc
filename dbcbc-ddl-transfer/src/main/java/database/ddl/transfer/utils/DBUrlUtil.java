...
public class DBUrlUtil {
		...
		switch (settings.getDataBaseType()) {
			case MYSQL:
				sb.append("jdbc:mysql://").append(settings.getIpAddress()).append(":").append(settings.getPort()).append("/").append(settings.getDataBaseName())
						.append("?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=Asia/Shanghai");
				break;
			case POSTGRESQL:
				...
				break;
			case ORACLE:
				if (settings.getOracleServiceName() != null && !settings.getOracleServiceName().trim().isEmpty()) {
					sb.append("jdbc:oracle:thin:@//").append(settings.getIpAddress()).append(":").append(settings.getPort()).append("/")
							.append(settings.getOracleServiceName());
				} else {
				...
				break;
			case DM:
				sb.append("jdbc:dm://").append(settings.getIpAddress()).append(":").append(settings.getPort()).append("/").append(settings.getDataBaseName());
				break;
			case SQLSERVER:
				sb.append("jdbc:sqlserver://").append(settings.getIpAddress()).append(":").append(settings.getPort())
					.append(";databaseName=").append(settings.getDataBaseName())
	...
	public static String generateDataBaseUrl(String databaseName, String ip, String port, String databaseType) {
		...
		switch (databaseType) {
			case "MYSQL":
				sb.append("jdbc:mysql://").append(ip).append(":").append(port).append("/").append(databaseName).append("?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=Asia/Shanghai");
				break;
			case "POSTGRESQL":
			...
			case "ORACLE":
				...
				break;
			case "DM":
				sb.append("jdbc:dm://").append(ip).append(":").append(port).append("/").append(databaseName);
				break;
			default:
				break;
		}
