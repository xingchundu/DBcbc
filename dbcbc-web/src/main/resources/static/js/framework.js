...
function initGlobalNavigation() {
    // 侧栏菜单：事件委托，避免子页面 ready 异常导致菜单无法绑定
    $(document).off('click.sidebarNav', '.sidebar-item[url]').on('click.sidebarNav', '.sidebar-item[url]', function (e) {
        e.preventDefault();
        $('.sidebar-item').removeClass('active');
    ...
    $dropdownBtn.off('click.dropdown').on('click.dropdown', function (e) {
        e.stopPropagation();
    ...
    $(document).off('click.dropdownClose').on('click.dropdownClose', function (event) {
...
$(function () {
    try {
        ...
        console.error('framework 初始化失败:', e);
        try {
            initGlobalNavigation();
        } catch (navErr) {
            console.error('导航初始化失败:', navErr);
        }
    }
});