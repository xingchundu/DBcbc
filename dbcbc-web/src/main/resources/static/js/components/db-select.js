...
(function(window, $) {
    ...
    $.fn.dbSelect = function(options) {
        ...
        const selectId = 'dbcbc-select-' + timestamp + '-' + random;
        const dropdownId = 'dbcbc-select-dropdown-' + timestamp + '-' + random;
        
        // 从原生 select 的类名中提取 select-mini，传递到生成的组件
        const miniClass = $select.hasClass('form-control-mini') ? 'dbcbc-select-mini' : '';
        
        const selectHTML = `
            <div class="dbcbc-select ${config.disabled ? 'disabled' : ''} ${miniClass}" id="${selectId}">
                <div class="dbcbc-select-trigger" data-toggle>
                    <div class="dbcbc-select-trigger-content">
                        <span class="dbcbc-select-trigger-text">${config.type === 'single' ? '请选择' : '请选择'}</span>
                        <div class="dbcbc-select-trigger-tags"></div>
                    </div>
                    <div class="dbcbc-select-arrow">
                        <i class="fa fa-angle-down"></i>
                    </div>
                </div>
                <div class="dbcbc-select-dropdown hidden" id="${dropdownId}">
                    <div class="dbcbc-select-search-wrapper">
                        <div class="dbcbc-select-search">
                            <i class="fa fa-search"></i>
                            <input type="text" class="dbcbc-select-search-input" placeholder="搜索...">
                            <div class="dbcbc-select-search-clear"><i class="fa fa-times"></i></div>
                        </div>
                    </div>
                    <div class="dbcbc-select-actions"></div>
                    <div class="dbcbc-select-options"></div>
                    <div class="dbcbc-select-empty hidden">暂无数据</div>
                </div>
            </div>
        `;
        ...
        const $trigger = $component.find('.dbcbc-select-trigger');
        const $dropdown = $component.find('.dbcbc-select-dropdown');
        const $searchInput = $component.find('.dbcbc-select-search-input');
        const $searchClear = $component.find('.dbcbc-select-search-clear');
        const $options = $component.find('.dbcbc-select-options');
        const $actions = $component.find('.dbcbc-select-actions');
        const $empty = $component.find('.dbcbc-select-empty');
        const $tags = $component.find('.dbcbc-select-trigger-tags');
        const $text = $component.find('.dbcbc-select-trigger-text');
        ...
        function renderOptions(filterText = '') {
            ...
            config.data.forEach(function(item, index) {
                ...
                // 单选时隐藏复选框，多选时显示
                const checkboxClass = config.type === 'single' ? 'dbcbc-select-option-checkbox hidden' : 'dbcbc-select-option-checkbox';

                const $option = $(`
                    <label class="dbcbc-select-option ${matches ? '' : 'hidden'} ${isSelected ? 'selected' : ''} ${item.disabled ? 'disabled' : ''}" title="${escapeHtml(item.label)}">
                        ...
                        <span class="dbcbc-select-option-text">${escapeHtml(item.label)}</span>
                    </label>
                `);
                ...
                if (config.type === 'single') {
                    ...
                    if (labels.length <= displayCount) {
                        labels.forEach(function(label) {
                            const $tag = $(`<span class="dbcbc-select-tag">${escapeHtml(label)}<i class="fa fa-times dbcbc-select-tag-remove"></i></span>`);
                            $tag.find('.dbcbc-select-tag-remove').on('click.dbSelect-' + selectId, function(e) {
                                e.stopPropagation();
                        ...
                        $tags.append(`<span class="dbcbc-select-count">+${count}</span>`);
                    }
                }
            } else {
                // 没有选中任何值时显示提示文字
        ...
        function adjustDropdownPosition() {
            ...
            // 移除之前的定位类
            $dropdown.removeClass('dbcbc-select-dropdown-up');
            
            // 如果下方空间不足，且上方空间足够，则向上展开
            if (spaceBelow < dropdownHeight && spaceAbove > spaceBelow) {
                $dropdown.addClass('dbcbc-select-dropdown-up');
            }
        }
        ...
        if (config.type === 'multiple') {
            // 添加默认按钮
            const $selectAllBtn = $(`<button class="dbcbc-select-action-btn">全选</button>`);
            const $deselectAllBtn = $(`<button class="dbcbc-select-action-btn">取消全选</button>`);
        ...
        config.customButtons.forEach(function(btn, index) {
            const $btn = $(`<button class="dbcbc-select-action-btn">${escapeHtml(btn.text)}</button>`);
